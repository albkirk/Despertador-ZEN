...
uint16_t WarnColor=GxEPD_WHITE;
uint16_t ECOColor=GxEPD_WHITE;
uint16_t DebugColor=GxEPD_WHITE;
...
void display_text(String text, uint16_t color=MainColor, uint8_t s=1, int x=0, int y=0, const GFXfont *font = NULL, bool wraptext=false) {
    ...
    //telnet_println(text +  " " + String(x) +  ", " + String(y) + ", " + String(tbx) + ", " + String(tby) + ", " + String(tbw) + ", " + String(tbh));
...
void display_drawbell(strDateTime strDtTm=DateTime, uint16_t color=MainColor, int x=(Time_x + T_NN), int y=40) {
      display_Image(bell_icon, x + 104, y, 16, 16, color);
      if (!strDtTm.alarm) {
...
void batt_icon_update() {
    ...
    if (Batt_Level > 100)  {
        BattPowered = false;
        display_Image(battery_icon_1_4, Display_Width - 20, 32, 16, 8, ECOColor);
    ...
    if (Batt_Level > 75 && Batt_Level <= 100)  {
        BattPowered = true;
        display_Image(battery_icon_1_4, Display_Width - 20, 32, 16, 8, MainColor);
